      ...
      <div className="flex-1 overflow-y-auto p-5">
        ...
        <div className="grid grid-cols-4 gap-3 mb-5">
          {[
            ...
            <div key={s.label} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
              ...
              <div className="text-[12px] text-slate-400 mt-0.5">{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
          <div className="flex items-center px-4 py-3 border-b border-slate-100">
            <div className="text-[13px] font-bold text-slate-900">Test Cases — {env.toUpperCase()}</div>
            {env === 'dev' && (
              <button className="ml-auto flex items-center gap-1.5 px-3 py-1.5 bg-[#0C7B8E] text-white rounded-lg text-[12px] font-medium cursor-pointer hover:bg-[#0A6577]">
                + Author TC
              </button>
            )}
          </div>
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Test Case</th>
                <th className="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Service</th>
                <th className="text-left px-4 py-2.5 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Tags</th>
            ...
            <tbody>
              {filtered.map((tc, i) => (
                <tr key={tc.id} className={`border-b border-slate-50 hover:bg-slate-50 ${i % 2 === 1 ? 'bg-slate-50/50' : ''}`}>
                  <td className="px-4 py-3 text-[13px] text-slate-700 font-medium">{tc.name}</td>
                  <td className="px-4 py-3">
                    <span className="text-[11px] px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-200 rounded-full font-medium">{tc.service}</span>
                  </td>
                  <td className="px-4 py-3">
                    {tc.tags.map(tag => (
                      <span key={tag} className="text-[11px] px-2 py-0.5 bg-green-50 text-green-700 border border-green-200 rounded-full font-medium mr-1">{tag}</span>
                    ))}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-[11px] px-2 py-0.5 rounded-full font-semibold ${tc.status === 'pass' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                      {tc.status}
                    </span>
                  </td>
                </tr>
