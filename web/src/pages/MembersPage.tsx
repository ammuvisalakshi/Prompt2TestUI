    ...
    <div className="h-full overflow-y-auto p-5 bg-[#F5F7FA]">
      <div className="max-w-3xl">
        <div className="flex items-center justify-between mb-5">
          <div>
            <div className="text-[15px] font-bold text-slate-900">Team Members</div>
            <div className="text-[12px] text-slate-400 mt-0.5">{MEMBERS.length} members · Cognito SSO</div>
          </div>
          <button className="px-3.5 py-2 bg-[#0C7B8E] text-white rounded-lg text-[13px] font-medium cursor-pointer hover:bg-[#0A6577] flex items-center gap-1.5">
            + Invite member
          </button>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100">
                ...
                <th className="text-left px-4 py-3 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Access</th>
              </tr>
            </thead>
            <tbody>
              {MEMBERS.map((m, i) => (
                <tr key={m.email} className={`border-b border-slate-50 hover:bg-slate-50 ${i % 2 === 1 ? 'bg-slate-50/50' : ''}`}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-[13px] font-semibold text-slate-900">{m.name}</div>
                        <div className="text-[11px] text-slate-400">{m.email}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-[11px] px-2 py-0.5 rounded-full border font-medium ${ROLE_COLORS[m.role] || ''}`}>
                      {m.role}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-[12px] text-slate-500">{m.access}</td>
                </tr>
              ))}
            </tbody>
          </table>
